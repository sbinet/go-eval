...
func (a *exprInfo) compileCompositeLit(c *expr, ikeys []interface{}, vals []*expr) *expr {
	if c == nil {
		a.diag("invalid composite expression")
		return nil
	}
	for i, elmt := range vals {
		if elmt == nil {
			a.diag("nil argument (#%d)", i+1)
			return nil
		}
	}

	// helper function to handle literals
	massage_lit_ideal := func(ty Type, elts []*expr) bool {
		ok := true
		if !ty.isIdeal() {
			for i := 0; i < len(elts); i++ {
				if elts[i].t.isIdeal() {
					elt := elts[i].convertTo(ty)
					if elt == nil {
						a.diag("cannot convert literal %d (type %s) to type %s",
							i+1, elts[i].t.String(), ty.String())
						return false
					} else {
						elts[i] = elt
					}
	...
	elts := vals
	comp := a.newExpr(c.valType, "composite literal")
	switch ty := c.valType.lit().(type) {
	case *NamedType:
		//fmt.Printf("--namedtype [%s]\n", ty.String())
		comp = nil
	case *StructType:
		keys := ikeys
		sz := len(ty.Elems)
		if len(elts) > sz {
			a.diag("given too many elements (%d) (expected %d)",
				len(elts), len(ty.Elems))
			return nil
		}
		if len(elts) < sz {
			a.diag("too few values in struct initializer")
			return nil
		}
		if len(elts) != len(keys) {
			if len(keys) > 0 && len(keys) < len(elts) {
				a.diag("mixture of field:value and value initializers")
				return nil
			}
		}
		chk_and_apply_cnv := func(elts []*expr) bool {
			for i := 0; i < sz; i++ {
				if !ty.Elems[i].Type.isIdeal() && elts[i].t.isIdeal() {
					elt := elts[i].convertTo(ty.Elems[i].Type)
					if elt == nil {
						a.diag("cannot convert literal #%d (type %s) to type %s",
							i+1, elts[i].t.String(), ty.Elems[i].Type.String())
						return false
					} else {
						elts[i] = elt
					}
		...
		var eval_fct func(t *Thread) Value
		if len(keys) > 0 {
			if _, ok := keys[0].(string); !ok {
				a.diag("invalid key type '%T' (expected string)", keys[0])
				return nil
			}
			...
			if len(ty.Elems) < len(keys) {
				a.diag("too many values in struct initializer")
				return nil
			}
			...
			if !chk_and_apply_cnv(tmp) {
				return nil
			}
			elts = tmp
			eval_fct = func(t *Thread) Value {
				out := ty.Zero().(StructValue)
				for i := 0; i < sz; i++ {
		...
		comp.genValue(eval_fct)

	case *ArrayType:
		sz := len(elts)
		if int64(sz) > ty.Len {
			a.diag("given too many elements (%d) (expected %d)", sz, ty.Len)
			return nil
		}
		if !massage_lit_ideal(ty.Elem, elts) {
			return nil
		}
		eval_fct := func(t *Thread) Value {
			base := ty.Zero().(ArrayValue)
			for i := 0; i < sz; i++ {
	...
	case *SliceType:
		...
		arrt := NewArrayType(int64(sz), ty.Elem)
		eval_fct := func(t *Thread) Value {
			base := arrt.Zero().(ArrayValue)
			for i := 0; i < sz; i++ {
				base.Elem(t, int64(i)).Assign(t, elts[i].asValue()(t))
			}
			return &sliceV{Slice{base, int64(sz), int64(sz)}}
		}
		comp.genValue(eval_fct)

	case *MapType:
		if len(elts) != len(ikeys) {
			a.diag("internal logic error")
			return nil
		}
		sz := len(elts)
		keys := make([]*expr, len(ikeys))
		for i := 0; i < sz; i++ {
			k, ok := ikeys[i].(*expr)
			if !ok {
				a.diag("key #%d isnt a *expr! (got %T)", ikeys[i], ikeys[i])
				return nil
			}
			keys[i] = k
		}
		if !massage_lit_ideal(ty.Key, keys) ||
			!massage_lit_ideal(ty.Elem, elts) {
			return nil
		}
		eval_fct := func(t *Thread) Value {
			m := evalMap{}
	...
	if comp == nil {
		a.diag("composite literal not impletemented for type [%s]\n", c.valType.lit().String())
	}
	return comp
}
		...
		if t, ok := t.(*packageType); ok {
			for i, f := range t.Elems {
				...
				builder = func(parent *expr) *expr {
					...
					expr := a.newExpr(ft, "selector expression")
					pf := parent.asPackage()
					evalAddr := func(t *Thread) Value { return pf(t).Ident(t, index) }
					expr.genValue(evalAddr)
					return sub(expr)
				}
			}
...
func (a *exprInfo) compileKeyValueExpr(key, val *expr) *expr {
...
func (a *exprInfo) compileBuiltinCallExpr(b *block, ft *FuncType, as []*expr) *expr {
	...
	case appendType:
		...
		case *SliceType:
			elmty = t.Elem
		default:
			a.diag("illegal argument type for 'append' function\n\t%v", arg.t)
			return nil
		}
		...
		for i := range as[1:] {
			...
			if srct.isIdeal() {
				src = src.convertTo(elmty)
				if src == nil {
					a.diag("cannot convert argument %d (type %s) to type %s in 'append'",
						i+1, srct.String(), elmty.String())
					return nil
				}
				srct = src.t.lit()
			}
			...
			if !srct.compat(elmty, withconv) {
				a.diag("cannot use %v (type %s) as type %s in 'append'",
					as[i+1].desc,
					srct.String(),
					elmty.String())
				return nil
		...
		expr := a.newExpr(NewSliceType(elmty), "function call")
		inval := as[0].derefArray().asSlice()
		expr.eval = func(t *Thread) Slice {
			src_sz := inval(t).Len
			sz := src_sz + int64(len(as[1:]))
			dstt := NewArrayType(sz, elmty)
			base := dstt.Zero().(*arrayV)
			if src_sz > 0 {
				base.Sub(0, src_sz).Assign(t, inval(t).Base)
			}
			for i := range as[1:] {
				src := srcs[i]
				var val Value = src.asValue()(t)
				base.Elem(t, int64(i)+src_sz).Assign(t, val)
			}
			return Slice{base, sz, sz}
		}
		return expr
...
func (a *exprInfo) compilePackageImport(name string, pkg *PkgIdent, constant, callCtx bool) *expr {
	fields := make([]packageField, 0)
	values := make([]Value, 0)
	for k, v := range pkg.scope.defs {
		...
		case *NamedType:
			...
			fva = fty.Zero()
		case *PkgIdent:
			// ignore: do not export packages from an import
			continue
		default:
			log.Panicf("unhandled Def type (%T)", vv)
		}
		field := packageField{k, fty}
		fields = append(fields, field)
		values = append(values, fva)
	}
	pkgty := newPackageType(name, fields)
	pkg_expr := a.newExpr(pkgty, "package")
	idents := []*expr{}
	for _, f := range pkgty.Elems {
		idents = append(idents, a.compileIdent(pkg.scope.block, constant, callCtx, f.Name))
	}
	pkg_expr.eval = func(t *Thread) PackageValue {
		out := pkgty.Zero().(*packageV)
		out.name = name
		out.idents = values
		return out
	}
