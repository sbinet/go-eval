...
func (a *stmtCompiler) compileImportDecl(decl *ast.GenDecl) {
	for _, spec := range decl.Specs {
...
func findPkg(path string) (filename, id string) {
	...
	if err == nil && tree != nil && tree.HasSrc(path) {
		filename = filepath.Join(tree.SrcDir(), path)
		id = path
	}
	return
}
