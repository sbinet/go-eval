...
var exprTests = []test{
	...
	Val("ai[:2]", vslice{varray{1, 2}, 2, 2}),
	Val("ai[:]", vslice{varray{1, 2}, 2, 2}),

	Val("sli[0:2]", vslice{varray{1, 2, 3}, 2, 3}),
	Val("sli[0:i]", vslice{varray{1, 2, 3}, 1, 3}),
	Val("sli[1:]", vslice{varray{2, 3}, 1, 2}),
	Val("sli[:2]", vslice{varray{1, 2, 3}, 2, 3}),
	Val("sli[:]", vslice{varray{1, 2, 3}, 2, 3}),
	...
	Val("append(sli, 3)", vslice{varray{1, 2, 3}, 3, 3}),
	Val("append(sli, 3, 4)", vslice{varray{1, 2, 3, 4}, 4, 4}),
	//FIXME: implement ellipsis unpacking
	//Val("append(sli, []int{3,4}...)", vslice{varray{1, 2, 3, 4}, 4, 4}),
	Val("append(sli, 3.0)", vslice{varray{1, 2, 3}, 3, 3}),
	CErr("append(sli, 3.1)", "cannot convert argument 1 [(]type ideal float[)] to type int in 'append'"),
	CErr("append(sli, \"2\")", "cannot use string literal [(]type <string>[)] as type int in 'append'"),
	...
	Val("copy([]int{}, sli)", 0),
	Val("copy([]int{0,0,0}, sli)", 2),
	Val2("x := []int{0,0,0}; _ := copy(x, sli)",
		"x[0] == sli[0]", true, "x[1]==sli[1]", true),
	Val2("x := []int{0,0,99}; _ := copy(x, sli)",
		"x[0] == sli[0]", true, "x[2]==99", true),
	Val2("x := []int{90,91,92}; _ := copy(x[1:], sli)",
		"x[0] == 90", true, "x[1]==sli[0]", true),
	Val2("x := []int{90,91,92}; _ := copy(x[1:], sli)",
		"x[2] == sli[1]", true, "x[1]==sli[0]", true),
	...
	Val("len(sli[1:])", 1),
	Val("len(sli[2:])", 0),
	Val("len(map[int]int{})", 0),
	Val("len(map[int]int{1:1,2:2})", 2),
	Val("len(ami)", 2),
	CErr("len(0)", opTypes),
	CErr("len(i)", opTypes),
	...
	// composite literals
	Val2("x := [2]int{11,22}", "len(x)", 2, "cap(x)", 2),
	Val2("x := [2]int{11,22}", "x[0]", 11, "x[1]", 22),
	Val2("x := [...]int{11,22}", "len(x)", 2, "cap(x)", 2),
	Val2("x := [...]int{11,22}", "x[0]", 11, "x[1]", 22),
	Val2("x := []int{11,22}", "len(x)", 2, "cap(x)", 2),
	Val2("x := []int{11,22}", "x[0]", 11, "x[1]", 22),

	Val1("x := map[int] int{1:42}; i = x[1]", "i", 42),
	Val1(`x := map[string] int{"1":42}; i = x["1"]`, "i", 42),
	Val1(`x := map[int] string{1:"42"}`, "x[1]", "42"),
	Val1("x := map[int] int{1:42}; i = len(x)", "i", 1),
	Val1("x := map[int] int{};     i = len(x)", "i", 0),
	Val2("x := map[int] int{1:42}; i, y := x[1]", "i", 42, "y", true),
